...
function shouldBehaveLikeERC721(errorPrefix, owner, newOwner, approved, anotherApproved, operator, other) {
  ...
  context('with minted tokens', function() {
    beforeEach(async function() {
      await this.token.mint(1, {from: owner});
      await this.token.mint(1, {from: owner});
  ...
  describe('_mint(address, uint256)', function() {
    ...
    //   await expectRevert(
    //       this.token.mint(1, {from: ZERO_ADDRESS}), 'ERC721: mint to the zero address',
    //   );
    // });

    context('with minted token', async function() {
      beforeEach(async function() {
        ({logs: this.logs} = await this.token.mint(firstTokenId, {from: owner}));
      });
...
function shouldBehaveLikeERC721Enumerable(errorPrefix, owner, newOwner, approved, anotherApproved, operator, other) {
  ...
  context('with minted tokens', function() {
    ...
    describe('tokenByIndexOffChain', function() {
      ...
      it('returns right index after burning multiple tokens', async function() {
      ...
      [firstTokenId, secondTokenId].forEach(function(tokenId) {
        it(`returns all tokens after burning token ${tokenId} and minting new tokens`, async function() {
          ...
          await this.token.mint(1, {from: newOwner});
          await this.token.mint(1, {from: newOwner});
  ...
  describe('_mint(address, uint256)', function() {
    ...
    context('with minted token', async function() {
      beforeEach(async function() {
        ({logs: this.logs} = await this.token.mint(1, {from: owner}));
      });
